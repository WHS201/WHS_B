...
목표명은 1~50자, 목표금액은 1원~10억 원이며 현재 총자산보다 커야 한다. 초기 자산을 설정한 뒤 목표를 생성·수정할 수 있고, 목표일은 오늘 이후다. 사용자당 완료 목표를 포함하여 최대 5개다. 완료된 목표는 수정할 수 없고, 삭제할 수 있다. 목표는 특정 금융상품과 연결하지 않는다. 표시 달성률은 `현재 총자산 / 목표금액 × 100`을 0~100으로 제한하며 완료된 목표는 자산 하락 후에도 100이다. 초기 설정 전에 생성되어 초기 입금만으로 충족된 기존 목표는 완료·뱃지 평가에서 제외하고 `requires_target_update`로 알린다. 해당 목표를 현재 총자산보다 큰 금액으로 수정한 뒤에는 정상 달성을 평가한다. 이미 기록된 완료·뱃지는 소급 변경하지 않는다. 목표 생성·수정, 대시보드·목표 목록·내 뱃지 조회와 스냅샷 배치에서 목표 완료 및 뱃지 지급을 평가하므로 해당 조회가 완료 상태를 갱신할 수 있다.
